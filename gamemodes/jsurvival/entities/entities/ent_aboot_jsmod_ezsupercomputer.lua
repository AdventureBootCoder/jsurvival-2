...
AddCSLuaFile()
ENT.Type = "anim"
ENT.PrintName = "EZ Quantum Super Computer"
...
if(SERVER)then
	...
	function ENT:Think()
		...
		if self.NextResourceThink < Time then
			...
			if State == STATE_ON then
				local ComputePerMin = 10
				local ComputeProgress = (ComputePerMin/60) * JMod.EZ_GRADE_BUFFS[Grade]

				self:SetProgress(self:GetProgress() + ComputeProgress)
	...
	function ENT:ProduceResource()
		self:SetProgress(0)
		hook.Run("JSmod_SuperComputerCalculated", self, JMod.GetEZowner(self))
	end
